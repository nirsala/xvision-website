...
// ── קלון רדוד: להעמיק לפני שקוראים תאריכים ──────────────────────
// הריצות ב-CI מקבלות קלון רדוד (depth≈50). ב-git log של קלון כזה
// קומיט הגבול נראה כאילו הוא שהוסיף את כל העץ, ולכן כל קובץ שלא נגעו
// בו בחלון הרדוד מקבל את תאריך הגבול — ולא את תאריך השינוי האמיתי שלו.
// חלון הגבול מתקדם ביום בכל ריצה, ולכן ~50 כתובות שלא השתנו כלל קיבלו
// lastmod חדש בכל בנייה. זה אות שווא לגוגל בדיוק כמו נפילת ה-mtime
// שהתיקון הקודם בא למנוע. ההעמקה היא best-effort: אם אין רשת או
// הרשאה, ממשיכים כרגיל ומקבלים את ההתנהגות הישנה במקום כישלון בנייה.
try {
  const shallow = execFileSync('git', ['rev-parse', '--is-shallow-repository'],
    { cwd: ROOT, encoding: 'utf8' }).trim();
  if (shallow === 'true') {
    execFileSync('git', ['fetch', '--unshallow'],
      { cwd: ROOT, encoding: 'utf8', stdio: 'ignore', timeout: 180000 });
  }
} catch {
  // אין רשת / אין remote / git לא זמין — ממשיכים עם מה שיש
}
